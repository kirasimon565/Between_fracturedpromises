...
CommandOutcome installAppCommand(CommandContext ctx) {
  ...
  final Duration duration =
      ctx.namedDuration('duration', const Duration(milliseconds: 2200));

  if (ctx.engine.state.phone.hasApp(appId)) return CommandOutcome.next;

  ctx.engine.state.installApp(InstalledApp(
    id: appId,
    name: name,
    icon: icon,
  ...
  ctx.engine.emitEffect(AppInstallEffect(
    appId: appId,
    name: name,
    icon: icon,
    duration: duration,
  ...
  ctx.engine.emitEvent(EngineEvents.appInstalled,
      data: <String, Object?>{'app': appId});
...
CommandOutcome _notification(CommandContext ctx) {
  ...
  ctx.engine.emitEvent(EngineEvents.notificationPosted,
...
CommandOutcome _clearNotifications(CommandContext ctx) {
  ctx.engine.state.clearNotifications(
      appId: ctx.count > 0 ? ctx.id(0) : null);
  return CommandOutcome.next;
}
...
CommandOutcome _wallpaper(CommandContext ctx) {
  ctx.engine.state
      .updatePhone((PhoneState p) => p.copyWith(wallpaper: ctx.str(0)));
  return CommandOutcome.next;
}
...
CommandOutcome _badge(CommandContext ctx) {
  ...
  ctx.engine.state.updatePhone((PhoneState p) => p.copyWith(
        apps: p.apps
            .map((InstalledApp a) =>
                a.id == appId ? a.copyWith(badge: count) : a)
            .toList(),
      ));
  return CommandOutcome.next;
}
