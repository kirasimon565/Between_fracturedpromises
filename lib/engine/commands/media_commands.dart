...
List<CommandHandler> mediaCommands() => <CommandHandler>[
  const FunctionCommand(<String>['image', 'show_image', 'photo'], _image),
...
CommandOutcome _background(CommandContext ctx) {
  ...
  ctx.engine.emitEffect(
    GenericEffect('background', <String, Object?>{
      'asset': asset,
...
CommandOutcome _video(CommandContext ctx) {
  ctx.engine.emitEffect(
    GenericEffect('video', <String, Object?>{
      'asset': ctx.str(0),
  ...
  ctx.engine.emitEffect(
    VibrateEffect(
      milliseconds: ctx.has('duration')
          ? ctx
                .namedDuration('duration', const Duration(milliseconds: 150))
                .inMilliseconds
          : ctx.integer(0, 150),
