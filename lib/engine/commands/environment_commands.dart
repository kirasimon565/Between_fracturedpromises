...
CommandOutcome _scene(CommandContext ctx) {
  ...
  final String app =
      ctx.namedStr('app', CommandContextHelpers.appForScene(scene));
  final String theme = ctx.namedStr(
      'theme', app == 'makelove' ? 'secret' : 'safe');
...
CommandOutcome _date(CommandContext ctx) {
  ctx.engine.state
      .updatePhone((PhoneState p) => p.copyWith(date: ctx.str(0)));
  return CommandOutcome.next;
}

CommandOutcome _location(CommandContext ctx) {
  ctx.engine.state
      .updatePhone((PhoneState p) => p.copyWith(location: ctx.str(0)));
  return CommandOutcome.next;
}

CommandOutcome _weather(CommandContext ctx) {
  ctx.engine.state
      .updatePhone((PhoneState p) => p.copyWith(weather: ctx.str(0)));
  return CommandOutcome.next;
}
...
CommandOutcome _syncFail(CommandContext ctx) {
  ...
  ctx.engine.state
      .updatePhone((PhoneState p) => p.copyWith(networkState: 'unstable'));
  return CommandOutcome.next;
}
