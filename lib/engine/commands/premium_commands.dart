...
CommandOutcome _crystals(CommandContext ctx) {
  ...
  ctx.engine.state.updateWallet(
    (Wallet wallet) => amount > 0 ? wallet.earn(amount) : wallet.spend(-amount),
  );
...
CommandOutcome _requireCrystals(CommandContext ctx) {
  ...
  ctx.engine.emitEffect(
    StoreEffect(reason: 'require_crystals', requiredCrystals: amount),
  );
...
CommandOutcome _premiumUnlock(CommandContext ctx) {
  ...
  ctx.engine.state.updateWallet((Wallet w) => w.spend(cost, choiceId: id));
  ...
  ctx.engine.emitEffect(
    ToastEffect(ctx.namedStr('title', 'Unlocked'), icon: 'crystal'),
  );
  return CommandOutcome.next;
}
